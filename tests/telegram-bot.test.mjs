...
import { formatRecruitment, parseTelegramCommand, parseVoteHour } from "../lib/telegram-commands.ts";
...
test("parses custom recruitment commands without native Telegram polls", () => {
  ...
  assert.deepEqual(parseTelegramCommand("/start"), { name: "help", argument: "" });
...
test("lists participants for a created start time", () => {
  const text = formatRecruitment({ id: 7, scheduledDate: "2026-08-28", hour: 9, status: "open", targetCount: 10, votes: [
    { displayName: "재미", username: "zaemi" },
    { displayName: "부처", username: null },
  ] });
...
test("secures and persists webhook updates", async () => {
  const [route, migration, setup] = await Promise.all([
  ...
  assert.match(setup, /allowed_updates: \["message"\]/);
});
