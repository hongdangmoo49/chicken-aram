...
test("validates preferred position combinations", () => {
  assert.deepEqual(normalizePlayerPositions([]), []);
  assert.deepEqual(normalizePlayerPositions(["탱커", "브루저"]), ["탱커", "브루저"]);
  assert.deepEqual(normalizePlayerPositions(["올라운더"]), ["올라운더"]);
  assert.equal(normalizePlayerPositions(["올라운더", "탱커"]), null);
  assert.equal(normalizePlayerPositions(["탱커", "올라운더"]), null);
  assert.equal(normalizePlayerPositions(["탱커", "탱커"]), null);
  assert.equal(normalizePlayerPositions(["탱커", "브루저", "메이지"]), null);
