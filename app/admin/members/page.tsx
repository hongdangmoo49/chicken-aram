    ...
    <section className="member-panel panel">
      ...
      <div className="member-list">
        {members.map((member) => <div className="member-row" key={member.id}>
          ...
          {canManageRoles && member.role !== "super_admin" ? <form action="/api/admin/role" method="post">
            ...
            <button className="button primary" type="submit">권한 저장</button>
          </form> : <span className="role-badge">{roleLabels[member.role]}</span>}
        </div>)}
      </div>
    </section>
  </PageShell>;
