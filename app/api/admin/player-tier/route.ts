...
export async function POST(request: Request) {
  ...
  if (user.role === "user") return redirectWithToast(request, "/tiers", "error", "관리자 권한이 필요합니다.");
  if (!(await takeRateLimit("admin-write", user.id, 60, 600))) return redirectWithToast(request, "/tiers", "error", "관리 요청이 너무 많습니다. 잠시 후 다시 시도해 주세요.");
