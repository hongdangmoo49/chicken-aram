...
export async function POST(request: Request) {
  ...
  if (!userId || (role !== "user" && role !== "admin")) {
    return redirectWithToast(request, "/admin/members", "error", "변경할 멤버와 권한을 확인해 주세요.");
  }

  try {
    await setMemberRole(userId, role);
  } catch {
    return redirectWithToast(request, "/admin/members", "error", "멤버 권한을 변경하지 못했습니다.");
  }
  return redirectWithToast(request, "/admin/members", "success", role === "admin" ? "관리자로 임명했습니다." : "일반 사용자로 변경했습니다.");
}
