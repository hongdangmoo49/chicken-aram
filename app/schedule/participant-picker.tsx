  ...
  return <>
    ...
    <div className="participant-picker">
      {players.map((player) => {
        ...
        const coach = player.tier === coachTier;
        ...
        return <div className={`participant-option${disabled ? " disabled" : ""}`} key={player.id}>
          <label htmlFor={`${pickerId}-player-${player.id}`}><input checked={checked} disabled={disabled} id={`${pickerId}-player-${player.id}`} name="players" onChange={(event) => setSelected((current) => event.target.checked ? [...current, player.id] : current.filter((id) => id !== player.id))} type="checkbox" value={player.id} /><PlayerAvatar player={player} /><span><strong>{player.nickname}</strong><small>{coach ? "코치 · 대전 참가 제외" : `${playerTierLabel(player.tier)} · ${player.wins}승 ${player.losses}패 · ${player.points}점`}</small></span></label>
