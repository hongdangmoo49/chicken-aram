...
export async function ResultsContent({ page }: { page: number }) {
  ...
  return <PageShell active="results">
    ...
    <AdminOnly><section className="result-entry-panel panel"><div className="result-section-heading"><div><span className="eyebrow">RESULT ENTRY</span><h2>대전 결과 등록</h2></div><p>예정 대전을 선택해 결과를 확정하면 선수 승패가 자동 반영됩니다.</p></div><div className="match-list">{scheduled.length ? scheduled.map((match) => <div className="scheduled-match manageable" key={match.id}><MatchCard match={match} /><div className="match-admin-actions"><details><summary>결과 등록</summary><ResultForm match={match} participants={participants(match.id)} submitLabel="결과 저장" /></details></div></div>) : <p className="empty">결과를 등록할 예정 대전이 없습니다.</p>}</div></section></AdminOnly>
    ...
    <div className="match-list">{completed.length ? completed.map((match) => <div className="completed-match" key={match.id}><MatchCard match={match} /><AdminOnly><div className="match-admin-actions"><details><summary>결과 수정</summary><ResultForm match={match} participants={participants(match.id)} submitLabel="수정 저장" /></details></div></AdminOnly></div>) : <p className="empty panel">등록된 대전 결과가 없습니다.</p>}</div>
