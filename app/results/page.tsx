...
import { getMatchCounts, getMatchParticipants, getMatches, getMvpAwards, type Match } from "../../db/site-data";
import { calculateTeamRankScores } from "../../db/team-balance";
import { AdminOnly } from "../session-ui";
import { MatchCard, PageShell } from "../ui";
import { MvpVotingPanel } from "./mvp-voting-panel";
...
function ResultForm({ match, submitLabel }: { match: Match; submitLabel: string }) {
...
export async function ResultsContent({ page }: { page: number }) {
  const pageSize = 10;
  const [completed, scheduled, matchCounts] = await Promise.all([
  ...
  const awards = await getMvpAwards(completed.map((match) => match.id));
  const totalPages = Math.max(1, Math.ceil(matchCounts.completed / pageSize));
  const rankScores = (matchId: number) => calculateTeamRankScores(members.filter((member) => member.matchId === matchId));
  return <PageShell active="results">
    ...
    <MvpVotingPanel />
    <AdminOnly><section className="result-entry-panel panel"><div className="result-section-heading"><div><span className="eyebrow">RESULT ENTRY</span><h2>대전 결과 등록</h2></div><p>예정 대전을 선택해 결과를 확정하면 선수 승패와 MVP 투표가 시작됩니다.</p></div><div className="match-list">{scheduled.length ? scheduled.map((match) => <div className="scheduled-match manageable" key={match.id}><MatchCard match={match} teamRankScores={rankScores(match.id)} /><div className="match-admin-actions"><details><summary>결과 등록</summary><ResultForm match={match} submitLabel="결과 저장" /></details></div></div>) : <p className="empty">결과를 등록할 예정 대전이 없습니다.</p>}</div></section></AdminOnly>
    ...
    <div className="match-list">{completed.length ? completed.map((match) => <div className="completed-match" key={match.id}><MatchCard match={match} mvpAwards={awards.filter((award) => award.matchId === match.id)} teamRankScores={rankScores(match.id)} /><AdminOnly><div className="match-admin-actions"><details><summary>결과 수정</summary><ResultForm match={match} submitLabel="수정 저장" /></details></div></AdminOnly></div>) : <p className="empty panel">등록된 대전 결과가 없습니다.</p>}</div>
    {totalPages > 1 && <nav aria-label="대전 결과 페이지" className="pagination">{page > 1 ? <Link className="button ghost" href={page === 2 ? "/results" : `/results/page/${page - 1}`}>이전</Link> : <span />}{page < totalPages && <Link className="button ghost" href={`/results/page/${page + 1}`}>다음</Link>}</nav>}
  </PageShell>;
}

export default function ResultsPage() {
  return <ResultsContent page={1} />;
}
