import { getMatchParticipants, getMatches, getPlayers, type Match, type Player } from "../../db/site-data";
...
export default async function ResultsPage() {
  ...
  const completed = matches.filter((match) => match.status === "completed").sort((a, b) => Date.parse(b.scheduledAt) - Date.parse(a.scheduledAt));
  ...
  return <PageShell active="results">
    ...
    <div className="result-section-heading archive-heading"><div><span className="eyebrow">COMPLETED</span><h2>완료된 대전</h2></div><p>{completed.length}개의 결과</p></div>
