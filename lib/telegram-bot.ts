...
export async function sendTelegramMessage(chatId: number, text: string) {
  return callTelegram<{ message_id: number }>("sendMessage", { chat_id: chatId, text: text.slice(0, 4096) });
}

export async function editTelegramMessage(chatId: number, messageId: number, text: string) {
  return callTelegram("editMessageText", { chat_id: chatId, message_id: messageId, text: text.slice(0, 4096) }, true);
}
