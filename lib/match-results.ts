...
export function normalizeMatchResult(value: unknown): MatchResultInput | null {
  ...
  if (normalizedDateTime !== localDateTime || aScore === bScore || (winner !== "A" && winner !== "B") || (winner === "A") !== (aScore > bScore)) return null;
  return { playedAt: playedAt.toISOString(), aScore, bScore, winner };
}
